...
class WxUser {
    async WxLogin(ctx) {
        ...
        if (data && data.openid) {
            let userData = await wx_user_model_1.WxUserModel.findUser(data.openid);
            if (!userData) {
                userData = new wx_user_model_1.WxUserModel({
                    openid: data.openid,
    ...
    async updateUserInfo(ctx) {
        try {
            ...
            const userData = await wx_user_model_1.WxUserModel.findUser(until_1.default.verifywxToken(token));
            if (userData) {
                await wx_user_model_1.WxUserModel.updateUser(userData.openid, {
    ...
    async WxFeedBack(ctx) {
        ...
        try {
            const feedbackData = await wx_user_model_1.WxFeedBackModel.findFeedBack(openid);
            if (feedbackData) {
            ...
            const doc = await doc_model_1.WxDocModel.getWxInfoById(Number(docId));
            if (!doc) {
                ctx.fail("文档不存在");
                return;
            }
            const data = new wx_user_model_1.WxFeedBackModel({ openid, docId, autor, info });
            await data.save();
    ...
    async getViews(ctx) {
        ...
        try {
            const userData = await wx_user_model_1.WxUserModel.findOne({ openid });
            const docData = await wx_user_model_1.WxUserModel.findOne({ openid }).populate({
                path: "views",
                options: {
                    limit: 10,
                    skip: (page - 1) * 10,
