...
import { WxDocModel, WxTypeModel } from "../model/doc-model";
import { WxUserModel } from "../model/wx-user-model";
...
export default class DocController {
	...
	@request("get", "/getDocById")
	@summary("获取id文档")
	@query({ id: { type: "number", require: true } })
	async getDocById(ctx: Ctx): Promise<void> {
		const { id } = ctx.request.query;
		if (!id) return ctx.fail("id 不能为空");
		const doc = await WxDocModel.getWxInfoById(Number(id));
		if (doc) return ctx.success(doc, "查询成功");
		ctx.fail("查询失败");
	}
	...
	async getDocByType(ctx: Ctx): Promise<void> {
		...
		try {
			const data = await WxDocModel.getDocByType(
				Number(type),
				contentType as string,
				Number(page)
			);
			const pages = await WxDocModel.getDocNumByType(
				Number(type),
				contentType as string
			);
			ctx.body = {
	...
	async getReadDoc(ctx: Ctx): Promise<void> {
		try {
			...
			const data = await WxDocModel.getWxInfoById(Number(id));
			const user = await WxUserModel.findUser(
				UntilService.verifywxToken(token as string)
			);
			if (!data || !user) {
				ctx.fail("用户未登录");
				return;
