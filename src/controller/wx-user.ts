...
import { WxUserModel, WxFeedBackModel } from "../model/wx-user-model";
import { WxDocModel } from "../model/doc-model";
...
export default class WxUser {
	...
	async updateUserInfo(ctx: Ctx): Promise<void> {
		try {
			...
			const userData = await WxUserModel.findUser(
				UntilService.verifywxToken(token)
			);
			if (userData) {
