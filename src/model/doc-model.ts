...
class WxType {
	...
	public static async getWxTypeById(
		this: ReturnModelType<typeof WxType>,
		id: number
	) {
		return this.findOne({ id }, { __v: 0 });
	}

	public static async getWxInfoByType(
		this: ReturnModelType<typeof WxType>,
		type: number
	) {
...
class WxDoc {
	...
	//获取type类型的文档
	public static async getDocByType(
		this: ReturnModelType<typeof WxDoc>,
		type: number,
		contentType: string,
		page: number,
		limit: number = 10
		...
		return this.find(
			{ type, contentType, disabled: { $ne: 1 } },
			{ _v: 0, _id: 0 }
		)
			.sort({ creatAt: -1 })
			.limit(limit)
	...
	//获取某类型总条数
	public static async getDocNumByType(
		this: ReturnModelType<typeof WxDoc>,
		type: number,
		contentType: string
	) {
	...
	//通过id获取某文档
	public static async getWxInfoById(
		this: ReturnModelType<typeof WxDoc>,
		id: number
	) {
	...
	//删除某文档
	public static async delDocById(
		this: ReturnModelType<typeof WxDoc>,
		id: number
	) {
	...
	//获取最新文档
	public static getDocByTime(
		this: ReturnModelType<typeof WxDoc>,
		page: number = 1,
		limit: number = 10
	) {
		return this.find({ disabled: { $ne: 1 } }, { _v: 0, _id: 0 })
			.sort({ creatAt: -1 })
			.limit(limit)
			.skip((page - 1) * limit);
	}
